          ...
          builder: (context, state) {
            if (state is DetailOrderLoadedState) {
              return SingleChildScrollView(
                child: Container(
                  padding: EdgeInsets.symmetric(vertical: MediaQuery.of(context).size.height * 0.1, horizontal: MediaQuery.of(context).size.width * 0.08),
                  child: Column(
                    children: [
                      Center(child: Text('${state.detailOrderModel.data.block.mainInfo.name}', style: TextStyle(fontWeight: FontWeight.bold),)),
                      SizedBox(
